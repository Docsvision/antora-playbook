...
module.exports.register = function ({config}) {
    ...
    config && Object.keys(config).length ?  logger.info('Override default configuration with %s', JSON.stringify(config, null, 2)) : logger.info('Use the default configuration') ;
    ...
    this.once('contentClassified', ({ contentCatalog  }) => {
        ...
        contentCatalog.getFiles()
            .filter(file => file.src.family === 'page' || file.src.family === 'partial')
            .forEach(file => {
                try{
                    if (file.contents) {
                        ...
                        imageMatches.forEach(match => {
                            ...
                            const normalizedPath = imagePath.includes(":") ? imagePath.split(":").slice(-2).join(":") : imagePath;
        ...
        contentCatalog.getFiles()
            ...
            .forEach(img => {
                ...
                const imagePath = img.src.relative.toString();
                const imageModulePath = img.src.module + ':' + img.src.relative.toString();
                if(!(imageReferences.has(imagePath) || imageReferences.has(imageModulePath))){
                  if(! (Array.from(imageReferences).some(reference => reference === imagePath ||  reference === imageModulePath )) ){
